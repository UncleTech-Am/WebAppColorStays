...
{   
    ...
    public class PlanTypeController : Controller
    {
        ...
        public async Task<Tuple<int, List<CsPlanType>>> AllDataList(int? PgSize, int? PgSelectedNum)
        {
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.GetAsync("PlanType/index/" + CompID + "/" + PgSize + "/" + PgSelectedNum + "/" + UserID, HttpCompletionOption.ResponseHeadersRead))
                {
        ...
        public async Task<IActionResult> DateSearch(IFormCollection fc)
        {
            ...
            try //Pagination and List of data Code
            {
                ...
                using (HttpClient client = APIColorStays.Initial())
                {
                    ...
                    using (var response = await client.PostAsJsonAsync("PlanType/DateSearch/", cIndex))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsPlanType>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
        ...
        [HttpPost]
        public async Task<IActionResult> TableSearch(CsPlanType CsPlanType, IFormCollection fc)
        {
            bool Success = false;
            int PgSelectedNum = Convert.ToInt32(fc["PageNoSelected"]);
            int PgSize = Convert.ToInt32(fc["PageSize"]);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.PostAsync("PlanType/TableSearch/?PageSelectedNum=" + pagedata.Item2 + "&PageSize=" + pagedata.Item1, content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsPlanType>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> FilterSearch(CIndexSearchFilter indexsearchfilter, IFormCollection fc)
        {
            ...
            Dictionary<string, string> fields = getClassMember.GetPropertyName(CsPlanType);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
               indexsearchfilter.CurrentUserId = UserID;
                ...
                using (var response = await client.PostAsync("PlanType/FilterSearch", content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsPlanType>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("PlanType/details/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        return View("_DetailOrDelete",CsPlanType);
                    }
                    else
                    {
                        Tuple<CsPlanType, Response> data = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<CsPlanType, Response>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
        ...
        //POST: /PlanType/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
		public async Task<IActionResult> Create(CsPlanType CsPlanType)
        {       
            Title();
            ViewData["AnName"] = "Create";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            if (ModelState.IsValid)
            {
                ...
                else { return View("_CreateOrEdit", CsPlanType); }
            }
            return View("_CreateorEdit",CsPlanType);                
         }
        ...
        public async Task<ActionResult> Edit(string id)
        {
            ...
            return View(CsPlanType);
        }

                
        //POST: /PlanType/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(CsPlanType CsPlanType)
        {
            Title();
            ViewData["AnName"] = "Edit";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
						client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                        using (var response = await client.PostAsJsonAsync<CsPlanType>("PlanType/edit", CsPlanType))
                        {
                            var apiResponse = await response.Content.ReadAsStreamAsync();
                            if (!response.IsSuccessStatusCode)
                            {
        ...
        public async Task<IActionResult> VerifyData(string Id, string AnName)          
        {
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.PostAsync("PlanType/verifydata/" , content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (!response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> MarkStar(Dictionary<string, string> Id, int PageNo, int PageSize)
        {
            ...
            starUnstar.ControllerName = "PlanType";
