...
{   
    ...
    [SessionCheck]
    [Authorize]
    public class ReviewForController : Controller
    {
        private readonly Paging paging;

        public ReviewForController()
        {
            paging = new Paging();
        }
        ...
        public async Task<Tuple<int, List<CsReviewFor>>> AllDataList(int? PgSize, int? PgSelectedNum)
        {
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            var UserID = HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
            Tuple<int, List<CsReviewFor>> list;
            using (HttpClient client = APIColorStays.Initial())
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("ReviewFor/index/" + CompID + "/" + PgSize + "/" + PgSelectedNum + "/" + UserID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsReviewFor>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    }
                    else{ list = null; }
                }
            }
        ...
        {         
            ...
            GetClassMember<CsReviewFor> getClassMember = new GetClassMember<CsReviewFor>();
            CsReviewFor CsReviewFor = new CsReviewFor();
            ViewBag.List = new SelectList(getClassMember.GetPropertyDisplayName(CsReviewFor), "Value", "DisplayName");
            //Ends
            ...
            try //Pagination and List of data Code
            {
                ...
                Task<Tuple<int, List<CsReviewFor>>> ReturnDataList = AllDataList(pagedata.Item1, pagedata.Item2);//Give the List of data
        ...
        public async Task<IActionResult> DateSearch(IFormCollection fc)
        {
            ...
            try //Pagination and List of data Code
            {
                ...
                using (HttpClient client = APIColorStays.Initial())
                {
                    ...
                    using (var response = await client.PostAsJsonAsync("ReviewFor/DateSearch/", cIndex))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsReviewFor>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
        ...
        [HttpPost]
        public async Task<IActionResult> TableSearch(CsReviewFor CsReviewFor, IFormCollection fc)
        {
            bool Success = false;
            int PgSelectedNum = Convert.ToInt32(fc["PageNoSelected"]);
            int PgSize = Convert.ToInt32(fc["PageSize"]);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                CsReviewFor.CreatedBy = UserID;
                CsReviewFor.CompId = CompID;
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                StringContent content = new StringContent(JsonSerializer.Serialize(CsReviewFor), Encoding.UTF8, "application/json");
                using (var response = await client.PostAsync("ReviewFor/TableSearch/?PageSelectedNum=" + pagedata.Item2 + "&PageSize=" + pagedata.Item1, content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsReviewFor>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> FilterSearch(CIndexSearchFilter indexsearchfilter, IFormCollection fc)
        {
            ...
            Dictionary<string, string> fields = getClassMember.GetPropertyName(CsReviewFor);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
               indexsearchfilter.CurrentUserId = UserID;
                ...
                using (var response = await client.PostAsync("ReviewFor/FilterSearch", content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsReviewFor>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("ReviewFor/details/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
                        CsReviewFor = await System.Text.Json.JsonSerializer.DeserializeAsync<CsReviewFor>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        return View("_DetailOrDelete",CsReviewFor);
                    }
                    else
                    {
                        Tuple<CsReviewFor, Response> data = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<CsReviewFor, Response>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
        ...
        public async Task<IActionResult> CreateOrEdit(string Id)
        {
            ...
            {
                bool Success = false;
                var data = new CsReviewFor();
                using (HttpClient client = APIColorStays.Initial())
                {
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                    using (var response = await client.GetAsync("ReviewFor/edit/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        data = await System.Text.Json.JsonSerializer.DeserializeAsync<CsReviewFor>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
                        { Success = true; }
                        else { Success = false; }
                    }
        ...
        //POST: /ReviewFor/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
		public async Task<IActionResult> Create(CsReviewFor CsReviewFor)
        {       
            Title();
            ViewData["AnName"] = "Create";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            CsReviewFor.CompId = CompID;
            CsReviewFor.CreatedBy = UserID;
            CsReviewFor.ModifiedBy = UserID;
            CsReviewFor.Id = Guid.NewGuid().ToString();
            ViewData["ResponseName"] = "ShowValidation";
            CsReviewFor data = new CsReviewFor();
            if (ModelState.IsValid)
            {
                using (HttpClient client = APIColorStays.Initial())
                {
				    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                    StringContent content = new StringContent(JsonSerializer.Serialize(CsReviewFor), Encoding.UTF8, "application/json");
                    using (var response = await client.PostAsync("ReviewFor/create", content))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        if (response.IsSuccessStatusCode)
                        {
                ...
                else { return View("_CreateOrEdit", CsReviewFor); }
            }
            return View("_CreateorEdit",CsReviewFor);                
         }


        //GET: /ReviewFor/Edit/5
        [HttpGet]
        public async Task<ActionResult> Edit(string id)
        {
            ...
            return View(CsReviewFor);
        }

                
        //POST: /ReviewFor/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(CsReviewFor CsReviewFor)
        {
            Title();
            ViewData["AnName"] = "Edit";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
						client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                        using (var response = await client.PostAsJsonAsync<CsReviewFor>("ReviewFor/edit", CsReviewFor))
                        {
                            var apiResponse = await response.Content.ReadAsStreamAsync();
                            if (!response.IsSuccessStatusCode)
                            {
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("ReviewFor/deleteconfirmed/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> VerifyData(string Id, string AnName)          
        {
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.PostAsync("ReviewFor/verifydata/" , content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (!response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> MarkStar(Dictionary<string, string> Id, int PageNo, int PageSize)
        {
            ...
            starUnstar.ControllerName = "ReviewFor";
        ...
        public async Task<JsonResult> CheckDuplicationReviewFor(string Name, string NameAction, string Id)
        {
            bool Success = false;
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            ...
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("ReviewFor/CheckDuplicationReviewFor/" + Name + "/" + NameAction + "/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        Success = true;
