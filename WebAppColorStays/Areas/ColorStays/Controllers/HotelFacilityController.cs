...
{   
    ...
    public class HotelFacilityController : Controller
    {
        ...
        public async Task<Tuple<int, List<CsHotelFacility>>> AllDataList(int? PgSize, int? PgSelectedNum)
        {
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            var UserID = HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
            Tuple<int, List<CsHotelFacility>> list;
            using (HttpClient client = APIColorStays.Initial())
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("HotelFacility/index/" + CompID + "/" + PgSize + "/" + PgSelectedNum + "/" + UserID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsHotelFacility>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    }
                    else{ list = null; }
                }
            }
        ...
        {         
            ...
            GetClassMember<CsHotelFacility> getClassMember = new GetClassMember<CsHotelFacility>();
            CsHotelFacility CsHotelFacility = new CsHotelFacility();
            ViewBag.List = new SelectList(getClassMember.GetPropertyDisplayName(CsHotelFacility), "Value", "DisplayName");
            //Ends
            ...
            try //Pagination and List of data Code
            {
                ...
                Task<Tuple<int, List<CsHotelFacility>>> ReturnDataList = AllDataList(pagedata.Item1, pagedata.Item2);//Give the List of data
        ...
        public async Task<IActionResult> DateSearch(IFormCollection fc)
        {
            ...
            try //Pagination and List of data Code
            {
                ...
                using (HttpClient client = APIColorStays.Initial())
                {
                    ...
                    using (var response = await client.PostAsJsonAsync("HotelFacility/DateSearch/", cIndex))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsHotelFacility>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
        ...
        [HttpPost]
        public async Task<IActionResult> TableSearch(CsHotelFacility CsHotelFacility, IFormCollection fc)
        {
            bool Success = false;
            int PgSelectedNum = Convert.ToInt32(fc["PageNoSelected"]);
            int PgSize = Convert.ToInt32(fc["PageSize"]);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                CsHotelFacility.CreatedBy = UserID;
                CsHotelFacility.CompId = CompID;
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                StringContent content = new StringContent(JsonSerializer.Serialize(CsHotelFacility), Encoding.UTF8, "application/json");
                using (var response = await client.PostAsync("HotelFacility/TableSearch/?PageSelectedNum=" + pagedata.Item2 + "&PageSize=" + pagedata.Item1, content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsHotelFacility>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> FilterSearch(CIndexSearchFilter indexsearchfilter, IFormCollection fc)
        {
            ...
            Dictionary<string, string> fields = getClassMember.GetPropertyName(CsHotelFacility);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
               indexsearchfilter.CurrentUserId = UserID;
                ...
                using (var response = await client.PostAsync("HotelFacility/FilterSearch", content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsHotelFacility>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("HotelFacility/details/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
                        CsHotelFacility = await System.Text.Json.JsonSerializer.DeserializeAsync<CsHotelFacility>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        return View("_DetailOrDelete",CsHotelFacility);
                    }
                    else
                    {
                        Tuple<CsHotelFacility, Response> data = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<CsHotelFacility, Response>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
        ...
        //GET: /HotelFacility/CreateOrEdit
        [HttpGet]
        ...
        public async Task<IActionResult> CreateOrEdit(string Id)
        {
            ...
            {
                bool Success = false;
                var data = new CsHotelFacility();
                using (HttpClient client = APIColorStays.Initial())
                {
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                    using (var response = await client.GetAsync("HotelFacility/edit/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        data = await System.Text.Json.JsonSerializer.DeserializeAsync<CsHotelFacility>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
                        { Success = true; }
                        else { Success = false; }
                    }
        ...
        //POST: /HotelFacility/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
		public async Task<IActionResult> Create(CsHotelFacility CsHotelFacility)
        {       
            Title();
            ViewData["AnName"] = "Create";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            CsHotelFacility.CompId = CompID;
            CsHotelFacility.CreatedBy = UserID;
            CsHotelFacility.ModifiedBy = UserID;
            CsHotelFacility.Id = Guid.NewGuid().ToString();
            ViewData["ResponseName"] = "ShowValidation";
            CsHotelFacility data = new CsHotelFacility();
            if (ModelState.IsValid)
            {
                using (HttpClient client = APIColorStays.Initial())
                {
				    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                    StringContent content = new StringContent(JsonSerializer.Serialize(CsHotelFacility), Encoding.UTF8, "application/json");
                    using (var response = await client.PostAsync("HotelFacility/create", content))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        if (response.IsSuccessStatusCode)
                        {
                ...
                else { return View("_CreateOrEdit", CsHotelFacility); }
            }
            return View("_CreateorEdit",CsHotelFacility);                
         }


        //GET: /HotelFacility/Edit/5
        [HttpGet]
        public async Task<ActionResult> Edit(string id)
        {
            ...
            return View(CsHotelFacility);
        }

                
        //POST: /HotelFacility/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(CsHotelFacility CsHotelFacility)
        {
            Title();
            ViewData["AnName"] = "Edit";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    {
						client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                        using (var response = await client.PostAsJsonAsync<CsHotelFacility>("HotelFacility/edit", CsHotelFacility))
                        {
                            var apiResponse = await response.Content.ReadAsStreamAsync();
                            if (!response.IsSuccessStatusCode)
                            {
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("HotelFacility/deleteconfirmed/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> VerifyData(string Id, string AnName)          
        {
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.PostAsync("HotelFacility/verifydata/" , content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (!response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> MarkStar(Dictionary<string, string> Id, int PageNo, int PageSize)
        {
            ...
            starUnstar.ControllerName = "HotelFacility";
        ...
        public async Task<JsonResult> CheckDuplicationHotelFacility(string Name, string NameAction, string Id)
        {
            bool Success = false;
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            ...
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("HotelFacility/CheckDuplicationHotelFacility/" + Name + "/" + NameAction + "/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        Success = true;
