...
{   
    ...
    public class StateController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> SaveImage(string SId)
        {
            var TokenKey = Request.Cookies["JWToken"];
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                {
                    var fileName = "State-" + Guid.NewGuid().ToString().Replace("-", "").Substring(0, 5) + Path.GetExtension(file.FileName);
        ...
        {       
            ...
            if (ModelState.IsValid)
            {
                using (HttpClient client = APIColorStays.Initial())
                {
                    foreach (var file in files)
                    {
                        var fileName = CsState.Name + "-" + file.FileName + Path.GetExtension(file.FileName);

                        CsState.Image = fileName;
                        CsState.AltTag = CsState.Name + "-" + file.FileName;
                        if (file.Length > 0)
                        {
                            Task<string> TImgUpload = ryCsImage.UploadWebImages(file, fileName, TokenKey, "State");
                            Task.WaitAll(TImgUpload);
        ...
        public async Task<IActionResult> Edit(CsState CsState)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    return RedirectToAction("Index", new { PageCall = "ShowIxSh", CsState.Id });
                }
        ...
        public async Task<JsonResult> CheckDuplicationState(string Name, string NameAction, string Id)
        {
            bool Success = false;
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            ...
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("State/CheckDuplicationState/" + Name + "/" + NameAction + "/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        Success = true;
