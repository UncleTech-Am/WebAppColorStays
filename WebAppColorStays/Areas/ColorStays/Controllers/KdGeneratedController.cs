...
{   
    ...
    [SessionCheck]
    [Authorize]
    public class KdGeneratedController : Controller
    {
        private readonly Paging paging;
        private readonly RyCSImage ryCsImage;

        public KdGeneratedController()
        {
            paging = new Paging();
        ...
        public async Task<Tuple<int, List<CsKdGenerated>>> AllDataList(int? PgSize, int? PgSelectedNum)
        {
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            var UserID = HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
            Tuple<int, List<CsKdGenerated>> list;
            using (HttpClient client = APIColorStays.Initial())
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("KdGenerated/index/" + CompID + "/" + PgSize + "/" + PgSelectedNum + "/" + UserID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsKdGenerated>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    }
                    else{ list = null; }
                }
            }
        ...
        {         
            ...
            GetClassMember<CsKdGenerated> getClassMember = new GetClassMember<CsKdGenerated>();
            CsKdGenerated CsKdGenerated = new CsKdGenerated();
            ViewBag.List = new SelectList(getClassMember.GetPropertyDisplayName(CsKdGenerated), "Value", "DisplayName");
            //Ends
            ...
            try //Pagination and List of data Code
            {
                ...
                Task<Tuple<int, List<CsKdGenerated>>> ReturnDataList = AllDataList(pagedata.Item1, pagedata.Item2);//Give the List of data
        ...
        public async Task<IActionResult> DateSearch(IFormCollection fc)
        {
            ...
            try //Pagination and List of data Code
            {
                ...
                using (HttpClient client = APIColorStays.Initial())
                {
                    ...
                    using (var response = await client.PostAsJsonAsync("KdGenerated/DateSearch/", cIndex))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsKdGenerated>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
        ...
        [HttpPost]
        public async Task<IActionResult> TableSearch(CsKdGenerated CsKdGenerated, IFormCollection fc)
        {
            bool Success = false;
            int PgSelectedNum = Convert.ToInt32(fc["PageNoSelected"]);
            int PgSize = Convert.ToInt32(fc["PageSize"]);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                StringContent content = new StringContent(JsonSerializer.Serialize(CsKdGenerated), Encoding.UTF8, "application/json");
                using (var response = await client.PostAsync("KdGenerated/TableSearch/?PageSelectedNum=" + pagedata.Item2 + "&PageSize=" + pagedata.Item1, content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsKdGenerated>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> FilterSearch(CIndexSearchFilter indexsearchfilter, IFormCollection fc)
        {
            ...
            Dictionary<string, string> fields = getClassMember.GetPropertyName(CsKdGenerated);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
               indexsearchfilter.CurrentUserId = UserID;
                ...
                using (var response = await client.PostAsync("KdGenerated/FilterSearch", content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    list = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<int, List<CsKdGenerated>>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                    if (response.IsSuccessStatusCode)
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("KdGenerated/details/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
                        CsKdGenerated = await System.Text.Json.JsonSerializer.DeserializeAsync<CsKdGenerated>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        return View("_DetailOrDelete",CsKdGenerated);
                    }
                    else
                    {
                        Tuple<CsKdGenerated, Response> data = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<CsKdGenerated, Response>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
        ...
        public async Task<IActionResult> CreateOrEdit(string Id)
        {
            ...
            {
                bool Success = false;
                var data = new CsKdGenerated();
                using (HttpClient client = APIColorStays.Initial())
                {
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                    using (var response = await client.GetAsync("KdGenerated/edit/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        data = await System.Text.Json.JsonSerializer.DeserializeAsync<CsKdGenerated>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        if (response.IsSuccessStatusCode)
                        { Success = true; }
                        else { Success = false; }
                    }
        ...
        //POST: /KdGenerated/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
		public async Task<IActionResult> Create(CsKdGenerated CsKdGenerated)
        {       
            Title();
            ViewData["AnName"] = "Create";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            CsKdGenerated.Id = Guid.NewGuid().ToString();
            ViewData["ResponseName"] = "ShowValidation";
            CsKdGenerated data = new CsKdGenerated();
            ...
            if (ModelState.IsValid)
            {
                using (HttpClient client = APIColorStays.Initial())
                {
                    foreach (var file in files)
                    {
                        ...
                        {
                            CsKdGenerated.CoverImage = fileName;
                        }
                    ...
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                    StringContent content = new StringContent(JsonSerializer.Serialize(CsKdGenerated), Encoding.UTF8, "application/json");
                    using (var response = await client.PostAsync("KdGenerated/create", content))
                    {
                        var apiResponse = await response.Content.ReadAsStreamAsync();
                        if (response.IsSuccessStatusCode)
                        {
                ...
                else { return View("_CreateOrEdit", CsKdGenerated); }
            }
            return View("_CreateorEdit",CsKdGenerated);                
         }


        //GET: /KdGenerated/Edit/5
        [HttpGet]
        public async Task<ActionResult> Edit(string id)
        {
            ...
            return View(CsKdGenerated);
        }

                
        //POST: /KdGenerated/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(CsKdGenerated CsKdGenerated)
        {
            Title();
            ViewData["AnName"] = "Edit";
            var TokenKey = Request.Cookies["JWToken"];
            ...
            CsKdGenerated.ModifiedBy = UserID;
            var files = HttpContext.Request.Form.Files;
            if (CsKdGenerated.CoverImageName != null)
            {
                CsKdGenerated.CoverImage = CsKdGenerated.CoverImageName;
            }
            if (ModelState.IsValid)
            {
                try
                {
                    using (HttpClient client = APIColorStays.Initial())
                    {
                        foreach (var file in files)
                        {
                            ...
                            Task<string> TDeleteImage = ryCsImage.DeleteImage(CsKdGenerated.CoverImageName, TokenKey, "KdGenBanner");
                        ...
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                        using (var response = await client.PostAsJsonAsync<CsKdGenerated>("KdGenerated/edit", CsKdGenerated))
                        {
                            var apiResponse = await response.Content.ReadAsStreamAsync();
                            if (!response.IsSuccessStatusCode)
                            {
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            var TokenKey = Request.Cookies["JWToken"];
			var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            CsKdGenerated csKd = new CsKdGenerated();
            using (HttpClient client = APIColorStays.Initial())
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("KdGenerated/edit/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    csKd = await System.Text.Json.JsonSerializer.DeserializeAsync<CsKdGenerated>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                }
            }
            ...
            {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("KdGenerated/deleteconfirmed/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> VerifyData(string Id, string AnName)          
        {
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.PostAsync("KdGenerated/verifydata/" , content))
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    if (!response.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> MarkStar(Dictionary<string, string> Id, int PageNo, int PageSize)
        {
            ...
            starUnstar.ControllerName = "KdGenerated";
        ...
        public async Task<JsonResult> CheckDuplicationKdGenerated(string Name, string NameAction, string Id)
        {
            bool Success = false;
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            ...
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
                using (var response = await client.GetAsync("KdGenerated/CheckDuplicationKdGenerated/" + Name + "/" + NameAction + "/" + Id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        Success = true;
