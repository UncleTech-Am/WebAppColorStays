...
{   
    ...
    public class RestaurantVideoController : Controller
    {
        ...
        public RestaurantVideoController()
        {
            paging = new Paging();
        }
        ...
        private void Title()
        {
            ViewBag.Title = "Restaurant Video";
        }
        //Ends
        ...
        public async Task<Tuple<int, List<CsRestaurantVideo>>> AllDataList(int? PgSize, int? PgSelectedNum)
        {
            var TokenKey = Request.Cookies["JWToken"];
            var CompID = Process.Decrypt(Base64UrlEncoder.Decode(Request.Cookies["CompanyID"]));
            var UserID = HttpContext.User.FindFirstValue(ClaimTypes.NameIdentifier);
            Tuple<int, List<CsRestaurantVideo>> list;
        ...
        {         
            ...
            GetClassMember<CsRestaurantVideo> getClassMember = new GetClassMember<CsRestaurantVideo>();
            CsRestaurantVideo CsRestaurantVideo = new CsRestaurantVideo();
            ViewBag.List = new SelectList(getClassMember.GetPropertyDisplayName(CsRestaurantVideo), "Value", "DisplayName");
            //Ends
            ...
            try //Pagination and List of data Code
            {
                ...
                Task<Tuple<int, List<CsRestaurantVideo>>> ReturnDataList = AllDataList(pagedata.Item1, pagedata.Item2);//Give the List of data
        ...
        [HttpPost]
        public async Task<IActionResult> TableSearch(CsRestaurantVideo CsRestaurantVideo, IFormCollection fc)
        {
            bool Success = false;
            int PgSelectedNum = Convert.ToInt32(fc["PageNoSelected"]);
            int PgSize = Convert.ToInt32(fc["PageSize"]);
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                CsRestaurantVideo.CreatedBy = UserID;
        ...
        public async Task<IActionResult> FilterSearch(CIndexSearchFilter indexsearchfilter, IFormCollection fc)
        {
            ...
            Dictionary<string, string> fields = getClassMember.GetPropertyName(CsRestaurantVideo);
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            using (HttpClient client = APIColorStays.Initial())
            {
                ...
                using (var response = await client.GetAsync("RestaurantVideo/details/" + id + "/" + CompID, HttpCompletionOption.ResponseHeadersRead))
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        CsRestaurantVideo = await System.Text.Json.JsonSerializer.DeserializeAsync<CsRestaurantVideo>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
                        return View("_DetailOrDelete",CsRestaurantVideo);
                    }
                    else
                    {
                        Tuple<CsRestaurantVideo, Response> data = await System.Text.Json.JsonSerializer.DeserializeAsync<Tuple<CsRestaurantVideo, Response>>(apiResponse, new System.Text.Json.JsonSerializerOptions { IgnoreNullValues = true, PropertyNameCaseInsensitive = true });
        ...
        public async Task<IActionResult> CreateOrEdit(string Id)
        {
            ...
            {
                bool Success = false;
                var data = new CsRestaurantVideo();
                using (HttpClient client = APIColorStays.Initial())
                {
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", TokenKey);
        ...
        {       
            ...
            CsRestaurantVideo.CompId = CompID;
            CsRestaurantVideo.CreatedBy = UserID;
            CsRestaurantVideo.ModifiedBy = UserID;
            CsRestaurantVideo.Id = Guid.NewGuid().ToString();
            ViewData["ResponseName"] = "ShowValidation";
            CsRestaurantVideo data = new CsRestaurantVideo();
            if (ModelState.IsValid)
            {
                ...
                else { return View("_CreateOrEdit", CsRestaurantVideo); }
            }
            return View("_CreateorEdit",CsRestaurantVideo);                
         }
